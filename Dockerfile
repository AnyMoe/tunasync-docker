...
COPY --from=goBuilder /go/src/github.com/tuna/tunasync/cmd /usr/local/bin
...
ENTRYPOINT ["tunasync", "manager" "--config", "/etc/tunasync/manager.conf",";", "tunasync", "worker", "--config", "/etc/tunasync/worker.conf",";"]
